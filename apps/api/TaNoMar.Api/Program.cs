...
api.MapPut("/admin/settings", async (PlatformSettingsRequest request, ClaimsPrincipal principal, TaNoMarDbContext db, CancellationToken cancellationToken) =>
{
    ...
    await db.SaveChangesAsync(cancellationToken);
    return Results.Ok(new { showPartners = settings.ShowPartners, showLiveWebcams = settings.ShowLiveWebcams });
}).RequireAuthorization();
...
record PlatformSettingsRequest(bool? ShowPartners, bool? ShowLiveWebcams);
