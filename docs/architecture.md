...
- `/` carrega a landing pública; `/privacidade` e `/excluir-conta` também são públicas e não iniciam a sessão. O roteador de sessão e as páginas autenticadas são carregados sob demanda a partir de `/entrar` ou de uma rota interna. A home do aplicativo fica em `/app`.
...
- `Admin`: snapshot gerencial da home `/admin` (`GET /admin/dashboard`) com KPIs de contas, locais, assinaturas, uso e parceiros.
