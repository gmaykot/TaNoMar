...
- `Webcams`: `WebcamService` autoriza e vincula; `WebcamProviderCatalog` resolve o provider por `Provider` + `ExternalId`; `WindyWebcamProvider` pesquisa por proximidade; `YouTubeWebcamProvider` consulta live pelo link (só admin) e, se o vídeo já encerrou, lista as lives do canal. Câmeras parceiras futuras usariam `PartnerWebcamProvider` (não implementado). O admin liga ou desliga a feature em `/admin` (`PlatformSettings.ShowLiveWebcams`). Ver [features/webcams.md](features/webcams.md).
