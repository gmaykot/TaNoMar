...
- `Webcams`: `WebcamService` autoriza e vincula; `WebcamProviderCatalog` resolve o provider por `Provider` + `ExternalId`; `WindyWebcamProvider` pesquisa por proximidade; `YouTubeWebcamProvider` consulta live pelo link (só admin). Câmeras parceiras futuras usariam `PartnerWebcamProvider` (não implementado). O admin liga ou desliga a feature em `/admin` (`PlatformSettings.ShowLiveWebcams`). Ver [features/webcams.md](features/webcams.md).
