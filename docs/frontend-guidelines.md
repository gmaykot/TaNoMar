...
O foco do aplicativo (`pescador`, `surfista` ou `ambos`) é preferência de apresentação. Não recalcule a nota nem mude o ranking por causa do foco: só mostre ou oculte blocos e indicadores. Sem foco escolhido, a web interrompe o primeiro acesso em `/comecar`.

Câmeras ao vivo usam `hasLiveWebcams(user)` (`modules.liveWebcams`). Não compare `plan.code === 'capitao'` na interface.
