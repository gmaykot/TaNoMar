...
O ranking em `/ranking` ordena pela nota da API. A ênfase (vento, chuva, ondas) só reordena. Os filtros avançados (ícone ao lado da ênfase) recortam a lista por descrição (trecho do nome do local, sem acento e sem diferenciar maiúsculas), nota, tipo, região e exposição, sem recalcular a nota. A lista mostra os locais em lotes de 10; `entitlements.maxRankingSpots` limita o total exibido, e `0` mantém a lista sem limite. Se houver locais além da cota, “Mostrar mais” fica visível e bloqueado ao atingir o máximo. O dia escolhido na home ou no ranking fica em `?data=` e segue na troca entre Início e Ranking.
