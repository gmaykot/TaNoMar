...
A landing reutiliza logo, tokens, ícones Lucide, `PageHeader`, `DateSelector`, `ForecastPresentation`, `RankingList` e o comportamento PWA existentes. O hero deixa explícito o login com Google, o Free sem cartão e a cobertura inicial na Grande Florianópolis. A demonstração da Home e a do ranking reutilizam os mesmos componentes do aplicativo, em um recorte inerte, com dados ilustrativos de Pântano do Sul, Joaquina, Campeche e Armação. A seção de cobertura explica locais oficiais, pessoais e compartilhados, com as regiões norte, sul, leste, oeste, continente e ilhas.
...
A seção de planos mostra o Free como acesso inicial e os cartões dos planos pagos. A tabela de comparação com o Free na primeira coluna fica recolhida até o visitante pedir: o controle “Comparar os planos”, o item “Comparar planos” do menu no celular e o atalho do rodapé abrem essa tabela (`#comparacao-planos`). Além de preço e cotas, a tabela mostra os módulos de mar, diário, offline, vento ideal, comunidade, ênfase no ranking e câmeras, lidos do catálogo.

A explicação da nota segue a fórmula da API: vento e direção, ondas e período, chuva, horário e perfil costeiro. Maré, temperaturas, espécie e modalidade não entram no cálculo. A seção de dúvidas esclarece acesso Free, entrada com Google, cobertura inicial na Grande Florianópolis, locais pessoais, favoritos, vento ideal, uso offline da previsão salva, diferença de cotas entre Arrais e Mestre, alertas ativos, câmeras (stream de terceiros, sem responsabilidade pelas imagens e sem garantia de manutenção ou disponibilidade) e cancelamento (Conta → Gerenciar assinatura → Cancelar renovação, sem estorno).
