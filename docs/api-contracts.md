...
Planos pagos na interface: **Arrais** (`arrais`), **Mestre** (`premium`) e **Capitão** (`capitao`). O código `premium` permanece estável para contas já assinantes. Recursos qualitativos (mar, diário, votos, indicadores, offline) valem para qualquer plano diferente de `free`; cotas mudam por plano.
...
- `POST /community/reports/{id}/confirm` e `/contest`: um voto por usuário, só na assinatura. Free vê e cria relatos; o autor não vota no próprio. Sem plano pago a API responde `400` (`plan_required`).
...
- `POST /me/alerts`: cria `{ spotId, minimumScore, leadHours }`. Só assinatura; a cota é `Plan.MaxAlerts` e a antecedência aceita de 1 a 168 horas.
