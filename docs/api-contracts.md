...
- `GET /fishing-spots`: locais oficiais **habilitados** e compartilhados aprovados cuja região esteja nas preferências da conta, além de todos os locais pessoais do dono independentemente da região. Oficiais sem `isFreeDefault` só entram nos planos pagos. O DTO inclui `profile`, `visibility`, `isApproved`, `isOwner`, `isFavorite`, `isEnabled`, `isInRanking`, `seaOrientationDegrees`, `idealWindDirectionDegrees` (preferência pessoal ou `null`) e `hasLiveWebcam` (há câmera ativa; sem URL). Sem preferência do usuário, oficiais visíveis, compartilhados aprovados e locais pessoais do dono vêm `isEnabled = true`.
...
- `PUT /fishing-spots/{id}`: dono edita o local, mantendo a região obrigatória; a mesma regra de duplicidade vale contra os demais; compartilhar de novo volta para moderação. Se coordenadas, orientação do mar ou perfil costeiro mudarem, a API invalida o cache de previsão desse local e aquece de novo.
...
- `GET /fishing-spots/{id}/forecast`: dias de previsão do local visível e incluído nas regiões preferidas, além dos locais pessoais do dono independentemente da região. O item de ranking também inclui `isOwner` e `visibility`.
...
O contrato de métrica é uma união `{ state: "available", value }` ou `{ state: "locked", reason, requiredPlan }`. Maré também aceita `{ state: "unavailable" }`. Medidas visíveis (vento, onda, chuva, temperatura e maré) usam vírgula decimal em pt-BR (`9,8`, `5,3 km/h`, `0,66 m`). A Open-Meteo fornece tempo, mar e pressão. A tábua usa dados publicados pela Marinha, lidos por uma API comunitária — não é canal oficial da Marinha nem serve para navegação. Snapshot com horas permanece servível até `Fishing:CacheHours`, mesmo sem tábua; completar a tábua não renova esse prazo. Depois de `Fishing:WarmupIntervalHours` a API devolve o snapshot e renova a semana em background. Chave opcional: `TABUA_MARE_API_KEY` / `Fishing:TabuaMareApiKey`. O frontend valida esse wire e mapeia para os tipos da UI; nota e classificação nunca são recalculadas no cliente.
...
- `GET /admin/fishing-spots`, `POST /admin/fishing-spots`, `PUT` e `DELETE /admin/fishing-spots/{id}`: só Admin. CRUD dos locais oficiais do sistema (`visibility = official`). O cadastro usa os mesmos campos de um local pessoal (nome, descrição, lugar, região, coordenadas, orientação, perfil costeiro), mais `isActive` (habilitado no mapa) e `isFreeDefault` (também visível no plano Free). Oficiais existentes nascem habilitados e no Free. Desabilitado some do mapa, ranking, previsão, relatos e câmera para qualquer plano. Sem role Admin: `403`.
- `GET /admin/users`, `PUT /admin/users/{id}/plan`, `PUT /admin/users/{id}/active` e `PUT /admin/users/{id}/role`: só Admin. Lista contas, troca o plano (`free`, `arrais`, `premium` ou `capitao`; `mestre` é aceito como alias de `premium`), bloqueia ou libera o acesso e altera o cargo (`Admin` ou `User`). Não atribui plano desligado. A conta do bootstrap pode mudar de plano e não é bloqueada; o admin não bloqueia a si mesmo nem o último admin ativo. Bloquear revoga os refresh tokens. Só a conta inicial altera o cargo: não rebaixa a si mesma nem a própria conta inicial. O DTO inclui `canChangeRole`.
