...
- `GET /me` inclui `features.showPartners` (admin em `/admin/parceiros`, padrão `false`) e `features.showAppFocus` (admin na home administrativa, padrão `false`). Sem `showAppFocus`, a web trata o foco como pesca e não mostra a escolha.
...
- `GET/PUT /admin/settings`: só Admin. Lê e grava `{ showPartners, showAppFocus }`. Campos omitidos no PUT permanecem. Independente da vitrine pública.
