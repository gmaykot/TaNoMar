...
- Regras e cálculo de pesca pertencem à API; o frontend nunca recalcula a nota. O foco do usuário (`pescador`, `surfista` ou `ambos`) só mostra ou oculta informações e depende da flag admin `showAppFocus` (padrão desligada; default pesca).
...
- A home `/admin` mostra o painel gerencial com KPIs de contas, locais, assinaturas e uso (`GET /admin/dashboard`).
- Locais oficiais do sistema (CRUD, habilitação e padrão do plano Free) são configurados em `/admin/locais-sistema`. Ver `docs/features/official-spots.md`.
- Como chegar no detalhe do local: mapa do aparelho (carro/a pé) ou rumo no app (barco). Ver `docs/features/arrival.md`.
- Câmeras ao vivo (módulo `liveWebcams`, plano Capitão, `IWebcamProvider` / Windy e YouTube): `docs/features/webcams.md`.
