...
- Cobrança da assinatura (proposta, Asaas Checkout com cartão, Arrais/Mestre/Capitão anuais −20%, cancelar recorrência sem estorno): `docs/billing.md` e `docs/decisions/ADR-004-asaas-checkout.md`. Os códigos de plano vêm de `PlanRules` (`arrais`, `premium`, `capitao`). A cobrança automática ainda não está ligada.
